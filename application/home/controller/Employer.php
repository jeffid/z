...
class Employer extends Permit1
{
    ...
    function postUpdateinfo(){
        ...
        if (DB::table('user')->where('id',$id)->update($data)){
            session('userName', $data['username']); //更新用户实名
            session('userPosition', $data['position']); //更新用户职位
...
//            var_dump($env);die;
        ...
        try {
            ...
            $env['co_id'] = $co_id;
            $user['co_id'] = $co_id;
            $user['id'] = session('uid') ?? 1;//todo  把默认值删了
