...
class permissions extends Allow
{
    ...
    public function postAdmininsert()
    {
        ...
        $request = request();
        //封装需要添加数据
        $data = $request->only(['username', 'passwrod', 'rid']);
//        //添加时间
//        $data['addtime'] = time();
        //封装status
//        $data['status'] = 1;
...
//        $this->success("角色分配成功", "/permissions/index");
    ...
    public function postupdate()
    {
        ...
        $id = $request->param('id');
        //获取修改的值
        $data = $request->only(['passwrod','status']);
...
//        Db::table("role_node")->insertAll($d);
        ...
        if (Db::table("role_node")->insertAll($d)) {
    ...
    public function postRoleinsery()
    {
        ...
        if (Db::table("node")->insert($data)) {
            $this->success("数据添加成功","/permissions/adminpermissions" );
            // var_dump($data);
        } else {
            $this->error("数据添加失败");
            // var_dump($data);
